    ...
    class Program 
    {
        ...
        public static bool Recall; 
          ...
          static void Game_OnGameLoad(EventArgs args) 
          {
                  ...
                  Config.SubMenu("killsteal").AddItem(new MenuItem("ks_enabled", "State").SetValue(true));
                  Config.SubMenu("killsteal").AddItem(new MenuItem("ks_Q", "Javelin Toss").SetValue(true));
                  Config.SubMenu("killsteal").AddItem(new MenuItem("ks_dot", "Ignite").SetValue(true));
         ...
         private static void Game_OnGameUpdate(EventArgs args) 
          ...
          {
            var ks_Q = Config.Item("ks_Q").GetValue<bool>();
            var ks_dot = Config.Item("ks_dot").GetValue<bool>();


            {
                var Q1Enemy = ObjectManager.Get<Obj_AI_Hero>().Where(hero => hero.IsValidTarget(Q.Range) && hero.Health < Q.GetDamage(hero));

                if (Q.IsReady() && Q1Enemy.Count() > 0)
                    Q.Cast(Q1Enemy.ToArray()[0]);
            }
