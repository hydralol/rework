...
namespace GX_Zilean
{
    class Program
    {
        private const string localversion = "Updated for L# ";
        ...
        private static void SaveLives()
        {
            ...
            if (Config.Item("RSave").GetValue<bool>())
            {
                //Game.PrintChat(string.Format("Save Lives Started"));
                foreach (Obj_AI_Hero AChamp in ObjectManager.Get<Obj_AI_Hero>())
                {
                    ...
                    {
                        //      Game.PrintChat(string.Format("Ally Found {0}", AChamp.BaseSkinName));
                        if (Config.Item("Save" + AChamp.BaseSkinName).GetValue<bool>() && R.IsReady())
                        {
                            //        Game.PrintChat(string.Format("Save {0} Confirmed", AChamp.BaseSkinName));
                            if (AChamp.Health < (AChamp.MaxHealth * (Config.Item("HPPercent").GetValue<Slider>().Value * 0.01)))
                            {
                                if ((!AChamp.IsDead) && (!AChamp.IsInvulnerable))
                                {
                                    //              Game.PrintChat(string.Format("{0} Needs Saving", AChamp.BaseSkinName));
