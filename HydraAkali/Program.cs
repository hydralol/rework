    ...
    internal class Program
    {
        ...
        private static void Combo()
        {
            ...
            if (target != null)
            {
                ...
                {
                    if (DamageLib.getDmg(target, 0, DamageLib.StageType.FourthDamage) > (double)target.Health)
                    {
                        Q.CastOnUnit(target, true);
                    }
                }
            ...
            else if (useE && E.IsReady())
            {
                foreach (var minion in allMinions)
                {
                    if (minion.IsValidTarget(E.Range) &&
                        minion.Health < 0.75 * DamageLib.getDmg(minion, DamageLib.SpellType.E))
                    {
                        E.CastOnUnit(minion);
                        return;
                    }
